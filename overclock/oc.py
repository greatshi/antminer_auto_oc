...
def get_info(ip, auth):
	...
	return {'ip':ip, 'temp_max':temp_max, 'ant_data':ant_data, 'clock_set':clock_set}
...
def change_freq(miners, low_temp, high_temp, term):
	while True:
		for miner in miners:
			ip = miner['ip']
			auth = get_auth(ip, miner['username'], miner['password'])
